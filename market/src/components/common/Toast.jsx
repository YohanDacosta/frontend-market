...
const Toast = ({ errors }) => {
    ...
    useEffect(() => {
        errors?.errors && handleShowToast();
    }, []);

    return (
        <div className={`fixed right-1 top-1 transition-opacity duration-300 ${showToast ? 'opacity-100' : 'opacity-0'}`}>
            <div className="max-w-xs bg-red-500 text-sm text-white rounded-md shadow-lg  mb-3 ml-3" role="alert">
                <div className="flex p-4">
                    <ul className='flex-1 mx-4'>
                        {errors?.message?.map((ms, idx) =>
                            <li className="list-decimal" key={idx}>{ms}</li>
                        )}
                    </ul>
                    <div className="ml-1">
